...
RSpec.describe SlimpayClient::Base do
  ...
  describe '#generate_api_methods' do
    ...
    it 'generate methods with proper names each time' do
      allow_any_instance_of(SlimpayClient::Base).to receive(:connect_api_with_oauth)
      allow_any_instance_of(SlimpayClient::Base).to receive(:request_to_api) {methods.to_json}
  ...
  describe '#connect_api_with_oauth', vcr: {cassette_name: 'oauth'} do
    it "calls Slimpay's OAuth2 server and retrieve token" do
      allow_any_instance_of(SlimpayClient::Base).to receive(:request_to_api) {{plop: 'lorem'}.to_json}
      allow_any_instance_of(SlimpayClient::Base).to receive(:generate_api_methods)
      slim = SlimpayClient::Base.new
